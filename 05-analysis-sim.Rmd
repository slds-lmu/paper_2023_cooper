...
```{r}
plot_errors_boxplot <- function(res_long, true_effects, problem, z_method = "original") {
  res_long |>
    filter(iter %in% range(iter), xcol != "Noise", z_method == !!z_method) |>
...
```{r error-bp-A}
(pa <- plot_errors_boxplot(res_long, true_effects,  "sim_a"))
```

```{r error-bp-B}
(pb <- plot_errors_boxplot(res_long, true_effects,  "sim_b"))
```

```{r error-bp-C}
(pc <- plot_errors_boxplot(res_long, true_effects,  "sim_c"))
```

```{r error-bp-D}
(pd <- plot_errors_boxplot(res_long, true_effects,  "sim_d"))
```

```{r boxplot-err-z-orig}
((pa + pb) / (pc + pd)) + 
  plot_layout(guides = "collect") & 
  theme(legend.position = "bottom")
```

### With `z_method = "aligned"`

```{r error-bp-A-aligned}
(pa_aligned <- plot_errors_boxplot(res_long, true_effects,  "sim_a", z_method = "aligned"))
```

```{r error-bp-B-aligned}
(pb_aligned <- plot_errors_boxplot(res_long, true_effects,  "sim_b", z_method = "aligned"))
```

```{r error-bp-C-aligned}
(pc_aligned <- plot_errors_boxplot(res_long, true_effects,  "sim_c", z_method = "aligned"))
```

```{r error-bp-D-aligned}
(pd_aligned <- plot_errors_boxplot(res_long, true_effects,  "sim_d", z_method = "aligned"))
```

```{r boxplot-err-z-aligned}
((pa_aligned + pb_aligned) / (pc_aligned + pd_aligned)) + 
  plot_layout(guides = "collect") & 
  theme(legend.position = "bottom")
```
...
```{r rmse-errorbars-A}
res_long |>
  filter(iter %in% range(iter), xcol != "Noise", problem == "sim_a") |>
  select(-is_noise, -converged, -xcol) |>
  # join true betas from design tbl for nonzero-effects
  left_join(true_effects, by = c("problem", "beta", "x")) |>
  mutate(
  ...
  group_by(x, iter, beta, z_scale, z_method, problem) |>
  summarize(
    rmse = sqrt(mean(error^2)),
    rmse_sd = sd(error)
  ) |>
  ggplot(aes(x = x, y = rmse, color = iter, fill = iter)) +
  facet_grid(rows = vars(beta), cols = vars(z_method, z_scale)) +
  geom_errorbar(aes(ymin = rmse - rmse_sd, ymax = rmse + rmse_sd), size = 2, position = "dodge2") +
  scale_color_brewer(
    palette = "Dark2", aesthetics = c("color", "fill"), 
    labels = c(fwelnet_mt = "Multi-Task fwelnet", glmnet = "Cause-Specific glmnet"),
    name = NULL
