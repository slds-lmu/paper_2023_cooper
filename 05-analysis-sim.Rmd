...
res |>
  count(problem, z_scale, z_method, theta, t)
```
...
```{r tidy-results, cache = TRUE, dependson="get-results"}
...
lineplot_bt_res(res_long, true_effects, "sim_a", exclude_noise = TRUE)
lineplot_bt_res(res_long, true_effects, "sim_b", exclude_noise = TRUE)
lineplot_bt_res(res_long, true_effects, "sim_c", exclude_noise = TRUE)
lineplot_bt_res(res_long, true_effects, "sim_d", exclude_noise = TRUE)
```
...
```{r}
plot_errors_boxplot <- function(res_long, true_effects, problem, z_method = "original", z_scale = 1, iter_max = 2, t = 1) {
  
  # shoddy vectorization with auto-patchworking
  if (length(problem) > 1) {
    p_list <- purrr::map(
      problem, ~plot_errors_boxplot(
        res_long = res_long,
        true_effects = true_effects, problem = .x, z_method = z_method, 
        z_scale = z_scale, iter_max = iter_max, t = t        
        )
      )
  ...
  res_long |>
    filter(
      ...
      xcol != "Noise", z_method == !!z_method, z_scale %in% !!z_scale, t %in% !!t) |>
    ...
    facet_grid(rows = vars(beta), cols = vars(z_scale, theta, t), labeller = label_context) +
...
```{r error-bp-A}
(pa <- plot_errors_boxplot(res_long, true_effects,  "sim_a", z_scale = 1, z_method = "original"))
```

```{r error-bp-B}
(pb <- plot_errors_boxplot(res_long, true_effects,  "sim_b", z_scale = 1, z_method = "original"))
```

```{r error-bp-C}
(pc <- plot_errors_boxplot(res_long, true_effects,  "sim_c", z_scale = 1, z_method = "original"))
```

```{r error-bp-D}
(pd <- plot_errors_boxplot(res_long, true_effects,  "sim_d", z_scale = 1, z_method = "original"))
```

```{r error-bp-all, fig.width=18, fig.height=16}
plot_errors_boxplot(
  res_long, true_effects, names(sim_labels), z_scale = c(1, 100), z_method = "original", iter_max = 10, t = c(1, 100)
)

plot_errors_boxplot(res_long, true_effects, "sim_a", z_scale = c(1, 100), z_method = "original", iter_max = 1)
plot_errors_boxplot(res_long, true_effects, "sim_a", z_scale = c(1, 100), z_method = "original", iter_max = 2)
plot_errors_boxplot(res_long, true_effects, "sim_a", z_scale = c(1, 100), z_method = "original", iter_max = 5,  t = c(1, 100))
```
