...
```{r error-bp-A}
plot_errors_boxplot(res_long, true_effects, "sim_a", z_scale = c(1, 100), iter_max = 1, t = c(1, 50, 100), thresh = c(0, 1e-3))
```

```{r error-bp-B}
plot_errors_boxplot(res_long, true_effects, "sim_b", z_scale = c(1, 100), iter_max = 1, t = c(1, 50, 100), thresh = c(0, 1e-3))
```

```{r error-bp-C}
plot_errors_boxplot(res_long, true_effects, "sim_c", z_scale = c(1, 100), iter_max = 1, t = c(1, 50, 100), thresh = c(0, 1e-3))
```
...
plot_errors_boxplot(
  res_long, true_effects, names(sim_labels), z_scale = c(1), iter_max = 5, t = c(1, 100), thresh = c(0)
)
```
...
res_long |>
  ...
  group_by(problem, beta, iter, z_scale, t, thresh) |>
  summarize(
  ...
  kable(
    escape = FALSE, 
    caption = "Median error (true-estimated) for all effect/non-noise variable per cause and setting",
    col.names = c("z-scale", "t", "threshold", "fwelnet", "glmnet", "fwelnet", "glmnet")
  ) |>
  kable_styling(bootstrap_options = c("striped", "hover")) |>
  add_header_above(c(" " = 3, "Cause 1" = 2, "Cause 2" = 2)) |>
  pack_rows(sim_labels[["sim_a"]], 1, 13) |>
  pack_rows(sim_labels[["sim_b"]], 14, 26) |>
  pack_rows(sim_labels[["sim_c"]], 27, 39) |>
  pack_rows(sim_labels[["sim_d"]], 40, 52) |>
