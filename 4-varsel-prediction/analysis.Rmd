...
#tbl_coefs <- tidyr::hoist(res, "result", "coefs") |>
#  tidyr::unnest("scores")
...
scores_bladder <- scores_long |> 
  filter(problem == "bladder_geno", is.na(alpha) | alpha == 1)
...
scores_bladder |>
  ...
  #filter((model %in% c("fwelnet", "glmnet")) | alpha == 1) |>
  group_by(problem, model, times, metric, cause, type) |>
  summarize(
  ...
  mutate(type = case_when(
    ...
    type == "both" ~ "Clinical + Gene Expr.")
  ) |>
  ggplot(aes(x = times, y = mean_score, color = model, fill = model)) +
  facet_grid(
    rows = vars(metric), cols = vars(type), labeller = labeller(cause = label_both, metric = label_value),
    scales = "free"
  ) +
  geom_point(alpha = .2) +
...
(p_brier <- scores_bladder |>
  ...
  filter(!is.na(score)) |>
  filter(score <= 1, score >= 0) |>
  #filter(model == "fwelnet") |>
  #filter(model != "rfsrc" | mtry == 3000) |>
  #filter(model != "coxboost" | (stepno == 100 & penalty == 3000)) |>
  filter(cause == 1) |>
  group_by(model, times, metric, cause, type) |>
  summarize(
